...
class HistogramMarker(Series):
    ...
    def plot(self, mplSubplot, dateTimes, color):
        ...
        mplSubplot.tick(validDateTimes, values, color=colors)
...
class Subplot(object):
    ...
    def addCallback(self, label, callback, defaultClass=LineMarker):
        ...
        :type label: string.
        :param callback: A function that receives a :class:`pyalgotrade.tick.Ticks` instance as a parameter and returns a number or None.
        """
        self.__callbacks[callback] = self.getSeries(label, defaultClass)
